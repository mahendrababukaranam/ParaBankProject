...
test.describe('Open New Account Suite', () => {
  test('Open New Checking Account', async ({ page }) => {
    ...
    // Ensure 'CHECKING' is selected in the account type dropdown
    await expect(page.locator('#type')).toHaveValue('0');

    // Select an account with sufficient balance from the 'from account' dropdown
